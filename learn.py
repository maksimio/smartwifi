...
for i in range(1): # l - label, c - class, s - single
  ...
  t_start = datetime.datetime.now()
  accuracy, y_pred = ml_single(ml.CRNN, train_x ,train_y_ml, test_x, test_y_ml, False, True)
  t_stop = datetime.datetime.now()
  print_metrics(str(t_stop - t_start) + ' ' + str(i) + ') ---CRNN-S', accuracy, test_y_ml, y_pred)
  t_start = datetime.datetime.now()
  accuracy, y_pred = ml.CRNN(train_x ,train_y_ml, test_x, test_y_ml, True, True)
  t_stop = datetime.datetime.now()
  sum2 += print_metrics(str(t_stop - t_start) + ' ' + str(i) + ') ---CRNN-L', accuracy, test_y_ml, y_pred)
  t_start = datetime.datetime.now()
  accuracy, y_pred = ml.CRNN(train_x ,train_y_mc, test_x, test_y_mc, False, True)
  t_stop = datetime.datetime.now()
